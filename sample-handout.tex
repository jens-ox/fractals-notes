...
Ein metrischer Raum ist eine Menge \( X \) mit einer reellwertigen Funktion \( X \times X \to \R \). Diese misst den Abstand zwischen zwei Punkten \( x,y \in X \) und muss dazu folgende Axiome erfüllen (\( x,y,z \in X \)):
\begin{enumerate}
...
Mithilfe von Cauchy-Folgen lassen sich nun vollständige metrische Räume konstruieren. Insbesondere brauchen wir kompakte metrische Räume, eine Teilmenge der vollständigen metrischen Räume.
...
Wir erhalten den \textbf{Raum der Fraktale}
\begin{equation*}
  (\mathcal{H}(X), h)\text{.}
\end{equation*}
